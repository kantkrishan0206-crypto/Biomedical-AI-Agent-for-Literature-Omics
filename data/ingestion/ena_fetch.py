"""Legal ENA metadata ingestion utilities."""
...
async def fetch_metadata(query: str, limit: int = 10) -> list[IngestedRecord]:
    ...
    url = "https://www.ebi.ac.uk/ena/browser/api/search"
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.get(url, params={"term": query, "retmax": limit})
        payload = response.text[:2000]
    record = IngestedRecord(
        source="ENA",
        identifier=f"ena:{fingerprint(query)[:12]}",
        title=f"ENA metadata query: {query}",
        abstract=payload,
        source_url=url,
